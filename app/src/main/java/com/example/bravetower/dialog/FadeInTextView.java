...
//        }
    ...
    private void initAnimation() {
        ...
        textAnimation = ValueAnimator.ofInt(0, textSize - 1);
        //执行总时间就是每个字的时间乘以字数
        textAnimation.setDuration(textSize * duration);
        //匀速显示文字
    ...
    public FadeInTextView setTextString(String textString) {
        if (textString != null) {
            ...
            int line = (textCount - 1) / size;
            textSize = textCount + line;
            //存放单个字的数组
            arr = new String[textSize];
            for (int i = 0,j = 0; i < textCount; i++,j++) {
                ...
                arr[j] = textString.substring(i, i + 1);
            }
            initAnimation();
        }
